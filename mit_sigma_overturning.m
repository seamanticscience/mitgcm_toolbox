function [psi, psimask] = mit_sigma_overturning(v,vmask,dx,dz,addlayer)
%function [psi, psimask] = mit_sigma_overturning(v,vmask,dx,dz);
% overturning stream function for mitgcm model in density space, time slabs are handled as 
...
%   end
  ...
  if addlayer
    ...
    for ky=1:ny
       psi(ky,find(isfinite(psi(ky,:,k)),1,'last')+1,k)=0;
%       psi(ky,find(isfinite(psi(ky,:,k)),1,'first')-1,k)=0;
    end
  end
  end

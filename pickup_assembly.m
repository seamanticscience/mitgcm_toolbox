function [nt,nf] = pickup_assembly(fpat,vars, fout,fsize)
...
% it = 0;
% while frdone == 0
fnin=dir([fpat(1:end-7),'*']); % Looks for files: ftype.iter.t*
for it=1:length(fnin);
%   it = it + 1;
    fnm = fnin(it).name;
%   disp(fnm);
...
for ivar = 1:length(vars)
  ...
  if ierr == 0 
    ...
    for id = 1:length(mydim_names)
      if ~isempty([intersect(horiz_names,mydim_names{id})])
        has_horiz = 1;
      end
    end
...
for ii=1:length(all_ncf);
    fin=netcdf(all_ncf(ii).name,'nowrite');
    Z=fin{'Z'}(:);
    Xin=sort([Xin;fin{'X'}(:)]);
